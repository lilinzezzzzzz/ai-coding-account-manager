...
2. 支持通过 OpenAI 邮箱新增账号、删除非活动账号、激活和单账号刷新额度。
...
安全头、Host、Origin 和请求体边界均使用项目自定义 middleware，以便统一接入
`slog` 并避免记录 query string 或其他敏感数据。Chi 版本由
...
- Docker 镜像必须包含或可挂载可执行的 Codex CLI；缺失时启动前校验失败并给出
  明确错误。
- 原生运行和 Docker 运行均不主动打开浏览器；启动日志打印本地 URL。
...
| `internal/security` | Host、Origin 和请求边界 |
...
`config/app.example.json` 可提交到仓库，`config/app.json` 是本地配置文件并被
`.gitignore` 忽略。配置读取顺序是内置默认值、`config/app.json`；`CODEX_HOME`
仅作为 `codexHome` 未配置时的外部工具 fallback。
...
`storage_id` 直接使用 `account_id`。`account_id` 由 provider 适配层保证稳定、可作为
路径段且不包含邮箱等 PII。
...
- 用户直接访问 `http://127.0.0.1:43127/`。
- 服务只监听 loopback 地址，Docker 发布端口也只绑定宿主机 `127.0.0.1`。
- 所有请求先经过 Host 校验，只接受当前配置端口上的 `127.0.0.1` 或 `localhost`。
- 当前阶段不维护登录态或浏览器凭据。

### 8.3 写请求校验

状态变更只允许 `POST` 或 `DELETE`，并要求：
...
- public：首页、必要静态资源和只读 API。
- mutation：额外要求 Origin、JSON Content-Type 和 body limit。
...
1. 用户在前端输入 OpenAI 账号邮箱。
2. 后端根据邮箱生成稳定且不含 PII 的 `account_id`，并令 `storage_id = account_id`。
3. 在短事务外完成输入校验，在 DAO 边界 upsert 账号元数据。
4. 仅保存账号元数据；只输入邮箱时不创建真实 Codex 凭证目录。

只输入邮箱无法读取真实 Codex 额度；真实 provider 没有对应凭据时，刷新结果应显示
稳定 `unavailable` 状态。

#### 9.1.2 从当前 Codex 登录态导入账号

1. 后端使用活动 `CODEX_HOME` 启动 Codex app-server，调用 `account/read` 识别账号。
2. 根据 email 生成稳定且不含 PII 的 `account_id`，并令 `storage_id = account_id`。
3. 将活动 `CODEX_HOME/auth.json` 复制到账号隔离凭据目录。
4. 数据库只 upsert 账号元数据，并把该账号标记为当前 active。

导入接口不返回 token、完整 `auth.json` 或 app-server 原始响应。

### 9.2 单账号额度刷新

每个账号使用独立 `auth.json`，刷新时使用一次性临时 `CODEX_HOME`：

1. 在事务外创建临时 `CODEX_HOME`。
2. 只把账号隔离目录中的 `auth.json` 复制到临时 `CODEX_HOME`。
3. 使用临时 `CODEX_HOME` 启动 app-server。
4. 调用 `account/read { refreshToken: true }`。
5. 调用 `account/rateLimits/read`。
6. 停止 app-server，并只把刷新后的临时 `auth.json` 原子复制回账号隔离目录。
7. 删除临时 `CODEX_HOME`。
8. 在短事务中更新账号元数据和 usage snapshot。

账号隔离凭据目录长期只保存 `auth.json`。Codex 运行态 sqlite、installation、日志、
插件锁等文件必须留在临时目录，避免 `.credentials` 混入非凭据状态。

MVP 仅由用户在账号列表中手动触发单账号刷新，不启动定时 scheduler。单账号刷新
失败不影响其他账号；失败状态也持久化，但保留最近一次有效 snapshot。

任何 app-server 或文件 I/O 都不能在数据库事务中执行。
...
| `POST` | `/api/providers/{providerId}/accounts/create` | 根据 OpenAI 邮箱新增本地账号 |
| `POST` | `/api/providers/{providerId}/accounts/import-current` | 从当前 Codex 登录态导入账号和隔离凭据 |
| `POST` | `/api/providers/{providerId}/accounts/{accountId}/activate` | 激活账号 |
| `POST` | `/api/providers/{providerId}/accounts/{accountId}/rename` | 更新账号展示名 |
| `DELETE` | `/api/providers/{providerId}/accounts/{accountId}` | 删除非活动账号 |
| `POST` | `/api/providers/{providerId}/accounts/{accountId}/usage/refresh` | 刷新单个账号额度 |
...
MVP 页面只显示 Codex 账号、套餐、额度、重置时间和状态。页面不展示 provider
available/unavailable 状态。账号通过输入 OpenAI 邮箱新增，额度刷新入口放在账号列表
单个账号卡片内。账号状态包括：
...
使用 typed `Config` 在启动时一次性读取和校验配置文件。默认读取
`config/app.json`，也可通过 `--config <path>` 指定其他配置文件。除 `CODEX_HOME`
作为 Codex 外部工具约定的 fallback 外，不使用环境变量作为正式配置入口。

| 配置字段 | 默认值 | 说明 |
| --- | --- | --- |
| `bindAddr` | `127.0.0.1:43127` | 本地监听地址 |
| `dataDir` | `.data` | SQLite 和登录任务运行数据目录 |
| `credentialsDir` | `.credentials` | 账号隔离凭据目录 |
| `codexBin` | 空 | Codex CLI；空值时自动发现 |
| `codexHome` | `CODEX_HOME` 或 `~/.codex` | 活动 Codex 状态目录；配置文件优先于 `CODEX_HOME` |
| `providerMode` | 空 | 设为 `fake` 时使用 fake provider |

约束：

- MVP 阶段 `bindAddr` 只接受 `127.0.0.1` 或 `localhost`。
- 启动监听前校验 CLI、数据目录、数据库和全部配置。
- controller、model 和 DAO 不直接读取配置文件或环境变量。
- 不提供数据库路径指向网络文件系统的受支持配置。
- post-MVP Docker 运行时不允许通过环境变量传入 token 或完整 `auth.json`。
...
- 单账号 usage refresh 使用请求 `context.Context`。
...
- HTTP handler 不启动脱离请求生命周期的 goroutine。
...
| refresh token 失效 | 标记账号不可用并保留最近 snapshot |
| 目标凭证无效 | 停止切换，不修改活动凭证 |
...
禁止记录 token、API key、完整凭证、app-server 原始报文、SQL 参数中的 PII、
环境变量或未脱敏异常对象。
...
2. 本地状态与凭证存储：实现 SQLite 初始化、SQL migration、model、DAO、
   ...
   和日志。
3. Codex 账号能力：实现 Codex app-server client、账号读取和手动刷新额度。
4. 账号操作 API：实现 Host、Origin、账号列表、邮箱新增账号、单账号刷新、
   激活和删除非活动账号。
...
| localhost 跨站请求 | Host/Origin、JSON Content-Type、CSP |
...
- 通过一个 Chi + GORM + SQLite 的无 CGO Go 二进制启动，无需安装 SQLite 或
  额外运行时。
- 服务仅通过本机 loopback 和 Host/Origin 请求边界访问。
- SQLite migration、约束、WAL 和 quick check 验证通过。
- SQLite 中不出现 token 或完整凭证。
- 通用服务层不直接解析 Codex `auth.json` 或调用 app-server。
- 三个账号可新增、手动刷新额度，并在单账号刷新失败时继续展示其他账号。
