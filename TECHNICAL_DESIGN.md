...
- ORM：GORM
- 数据库：SQLite
- 产品形态：前后端同仓的本地 Web 管理工具，支持单二进制或 Docker 启动，
  不实现为 VS Code 插件
- v1 provider：OpenAI Codex
- Codex 验证基线：Codex CLI `0.140.0-alpha.2`

项目用于统一管理本机 AI coding 工具账号。v1 支持保存多个 Codex ChatGPT
账号、查看额度、重新认证和切换活动账号。

通用层保持 provider-neutral，只处理账号生命周期、能力声明和标准化 usage
snapshot。Codex 的凭证格式、app-server 协议和 `CODEX_HOME` 规则必须封装在
`CodexProvider` 内。
...
1. 在浏览器中查看不同 provider 的账号和额度状态。
2. 支持账号新增、保存当前账号、重命名、重新认证、删除和激活。
3. 通过 provider adapter 获取可公开读取的账号和 usage 信息。
4. 使用 SQLite 可靠持久化账号元数据、活动状态、usage snapshot 和 schema 版本。
5. 确保 token 不进入数据库、项目仓库、浏览器存储、URL、日志或 API 响应。
6. 使用单个 Go 二进制或 Docker Compose 启动服务。
7. 新增 provider 时不迁移通用账号表、不修改 API envelope。
...
- 不采用传统 MVC 作为项目结构目标；后端按职责分为 handler、service、
  repository、provider 和 store。
...
- `gorm.io/gorm`：ORM、repository 查询和事务封装。
...
Chi 只允许存在于 transport 层；GORM 只允许存在于 database/repository 层。
service、provider 和 domain model 不依赖 `chi.Context`、`*gorm.DB` 或 GORM
model。项目不追求传统 MVC 架构，采用 handler/service/repository 的后端分层；
前端静态页面只作为本地管理 UI，不引入独立 View 层框架。
...
repository，前端资源放在 `internal/web/static`，通过 `//go:embed` 嵌入后端
二进制。前端通过同源 `fetch` 调用 API，不使用 `localStorage`、
...
默认监听 `127.0.0.1:43127`，生成进程级安全会话并自动打开浏览器。端口被占用
时直接失败。支持 `--no-open`。
...
- Docker 镜像必须包含或可挂载可执行的 Codex CLI；缺失时启动前校验失败并给出
  明确错误。
- 自动打开浏览器只适用于原生运行；Docker 运行默认等价于 `--no-open`，启动日志
  打印带 bootstrap token 的本地 URL。
...
| `internal/server` | `http.Server`、Chi Router、路由和静态文件 |
| `internal/handler` | `net/http` handler、request DTO、response envelope 和错误映射 |
| `internal/middleware` | 请求日志、recovery、安全头、Host、session 和 CSRF |
| `internal/service` | 编排账号生命周期、事务边界和稳定错误码 |
| `internal/database` | GORM 初始化、SQLite 配置、migration 和健康检查 |
| `internal/model` | 与 Chi/GORM 无关的 domain model |
| `internal/repository` | GORM repository、私有 persistence model 和错误映射 |
| `internal/store/credentials` | 凭证目录、权限、校验和原子替换 |
| `internal/provider` | provider contract、registry 和 capability |
| `internal/provider/codex` | Codex 认证、账号映射、usage 和切换 |
| `internal/appserver` | 子进程、JSON-RPC、通知、超时和退出处理 |
| `internal/scheduler` | 定时刷新、并发限制和 single-flight |
| `internal/security` | bootstrap、session、CSRF 和请求边界 |
| `internal/web` | 内嵌页面资源，不接触 token |

依赖通过构造函数显式注入，不使用可变 package-level singleton。

### 5.2 HTTP Handler 分层约束
...
HTTP Handler
  ...
  ├─ 调用 Service
  └─ domain error -> HTTP error envelope

Service
  ├─ 业务编排
  ├─ 事务边界
  └─ Provider / Repository / CredentialStore
```

handler 不直接执行 SQL、操作凭证文件或启动 app-server。service 方法接收
...
- handler 和 service 不持有 `*gorm.DB`。
- persistence model 与 API DTO、domain model 分离。
- repository 每次调用使用 `db.WithContext(ctx)`。
- service 通过 transaction manager 或 repository unit-of-work 明确开启事务。
...
GORM persistence model 显式映射表名和列名，不嵌入 `gorm.Model`，避免引入
...
- 将 `ErrRecordNotFound`、duplicated key、foreign key violation 和
  `SQLITE_BUSY` 映射为稳定 domain error。
...
- SQL 文件放在 `internal/database/migrations/` 并通过 `embed.FS` 打包。
...
- Docker 运行只允许容器内监听 `0.0.0.0`，宿主机端口发布必须绑定
  `127.0.0.1`。
...
或终止子进程并删除 pending 目录。重新登录仅允许相同 `account_id` 覆盖原凭证；
不同账号应作为新账号保存。
...
默认每 5 分钟刷新，并发上限为 2。使用 buffered channel 作为 semaphore，
`sync.Mutex` 保护 single-flight 状态。单账号失败不影响其他账号；失败状态也
持久化，但保留最近一次有效 snapshot。
...
GORM 实现内部使用 `db.WithContext(ctx).Transaction(...)`，向回调提供绑定同一个
`*gorm.DB` transaction 的 repository 集合。禁止在 service 中直接调用 GORM。
...
| `GET` | `/api/providers` | provider 状态与 capability |
| `GET` | `/api/accounts` | 账号和持久化 usage snapshot |
| `POST` | `/api/providers/{providerId}/accounts/import-current` | 保存当前账号 |
| `POST` | `/api/providers/{providerId}/accounts/{accountId}/activate` | 激活账号 |
| `POST` | `/api/providers/{providerId}/accounts/{accountId}/rename` | 重命名 |
| `POST` | `/api/providers/{providerId}/accounts/{accountId}/relogin` | 重新认证 |
| `DELETE` | `/api/providers/{providerId}/accounts/{accountId}` | 删除非活动账号 |
| `POST` | `/api/providers/{providerId}/login-tasks` | 新增账号认证 |
...
页面按 provider 分组显示账号、套餐、额度、重置时间和状态。账号状态包括：
...
| `AI_CODING_ACCOUNT_MANAGER_REFRESH_MINUTES` | `5` | 刷新间隔 |
...
- 端口范围为 `1024-65535`。
- `AI_CODING_ACCOUNT_MANAGER_BIND_ADDR` 只接受 `127.0.0.1` 或容器入口显式设置
  的 `0.0.0.0`；后者必须配合宿主机 loopback 端口发布。
- 刷新间隔范围为 `1-60` 分钟。
- 启动监听前校验 CLI、数据目录、数据库和全部配置。
- handler 和 repository 不直接读取环境变量。
- 不提供数据库路径指向网络文件系统的受支持配置。
- Docker 运行时不允许通过环境变量传入 token、完整 `auth.json` 或 OAuth URL。
...
- usage 刷新使用容量为 2 的 semaphore。
- scheduler 和 login task 使用父 `context.Context`，关闭时统一取消并等待。
...
- GORM query chain 不跨 goroutine 共享；每次 repository 调用从基础
  `*gorm.DB` 创建独立 session。
- HTTP handler 不启动脱离请求生命周期的 goroutine；后台任务必须交给受控
  scheduler/login task manager。
...
- GORM model 到 domain model 的映射、repository 查询和 error translation。
...
- provider registry、capability 和 Codex schema 映射。
- JSON-RPC 请求关联、通知、超时和子进程退出。
- Host、Origin、session、CSRF、请求体和字段校验。
- Chi route group 的 middleware 覆盖、404/405 和 panic recovery。
- handler 到 service 的参数、context 取消和 domain error 映射。
- 写操作互斥、刷新 single-flight 和并发限制。
- API、数据库和日志不包含敏感字段。
...
发布前额外运行目标平台交叉编译、二进制启动 smoke test 和 Docker 启动
smoke test。
...
凭证和运行数据位于 XDG data dir 或 Docker volume/bind mount，不属于项目交付物。
发布构建必须验证静态文件和 migration 已嵌入二进制，Docker build context 不包含
任何真实凭证、数据库、WAL 或备份文件。

### 14.3 实施顺序

1. 初始化 Go module、命令入口、配置和 shutdown 骨架。
2. 实现 GORM/SQLite 初始化、migration、repository、unit-of-work 和
   credential store。
3. 实现 provider contract、registry 和 Codex app-server client。
4. 实现账号 service、文件/数据库一致性与 usage scheduler。
5. 实现 Chi router、middleware、handler 和内嵌静态页面。
6. 补齐 Dockerfile、compose、README 启动说明和 Docker smoke test。
7. 完成 race test、协议只读验证、备份恢复验证和三账号人工验收。
...
| Docker 误暴露到局域网 | compose 固定 `127.0.0.1` 端口发布，启动时校验 Host/Origin |
| SQLite 单 writer | 单连接、短事务、busy timeout、事务外执行 I/O |
| GORM 隐式行为 | repository 隔离、禁用 AutoMigrate/Save、显式字段和事务 |
...
| token 进入 SQLite/WAL | schema 和 repository 禁止凭证字段，测试扫描敏感字段 |
