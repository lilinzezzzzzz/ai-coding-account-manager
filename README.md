...
Codex 账号，提供浏览器管理页面，用于登录添加账号、导入凭据、查看额度、重置额度、
刷新状态、切换活动账号和删除非活动账号。
...
- 支持 Codex browser 登录添加、选择 `auth.json` 文件导入、粘贴 JSON 内容导入。
- 支持刷新账号额度、消费 reset credit 重置额度、设置套餐到期日、激活账号和删除非活动账号。
...
`start` 和 `restart` 会在启动前检查当前分支 upstream 是否有远程更新。脚本只执行
`git fetch` 和落后提交数检查，不会自动 `git pull` 或修改本地代码；如果远程有新提交，
服务启动后会提示执行 `git pull --ff-only`。脚本输出在交互式终端中会使用颜色标识步骤、
成功、警告和错误；如需关闭颜色，可设置 `NO_COLOR=1`。
...
管理页面的“添加账号 -> 登录”会创建隔离的临时 `CODEX_HOME`，引导用户完成官方
Codex 登录，然后读取真实账号 email 和套餐信息，并把临时目录里的 `auth.json` 导入到
该账号的隔离凭据目录。
...
管理页面的“添加账号 -> 导入账号”支持两种方式：

- “文件”：选择本地 `auth.json` 文件导入。
- “JSON”：粘贴 `auth.json` 内容导入。

两种方式都会先在临时 Codex 运行目录中识别账号并刷新 usage，成功后才写入账号数据库
记录、最近 usage snapshot 和账号隔离凭据目录。选择文件时如果取消文件选择，不会关闭
导入对话框或清空已有内容。
...
账号卡片会在 provider 返回 reset credit 信息时显示额度重置按钮。可重置次数大于 0 时，
点击按钮会二次确认并消费 1 次 reset credit；可重置次数为 0 时按钮不可点击。后端在调用
provider 重置前也会校验最近 usage snapshot 中的 `rateLimitResetCredits.availableCount`
大于 0；缺少可验证的 reset credit 数据时，需要先刷新账号状态。
...
POST   /api/providers/{providerId}/accounts/{accountId}/rate-limit/reset
...
frontend/static/components/     原生 JS DOM 组件
...
- reset credit 大于 0 的账号可以完成额度重置；reset credit 为 0 时前端按钮不可点击，
  后端也拒绝直接重置。
